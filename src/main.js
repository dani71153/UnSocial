...
const store = new Store({
  defaults: {
    ...
    allLabels: [],   // Global list of user-defined labels
    httpCacheEnabled: false, // When false, feeds are served with Cache-Control: no-store
    windowState: {
      width: 960,
      height: 700,
      x: null,
...
ipcMain.handle('set-feed-labels', (_e, username, platform, labels) => {
  const feeds = store.get('feeds').map((f) => {
    if (f.username === username && (f.platform || 'instagram') === (platform || 'instagram')) {
      return { ...f, labels };
    }
...
ipcMain.handle('set-feed-filters', (_e, username, platform, filters) => {
  const feeds = store.get('feeds').map((f) => {
    if (f.username === username && (f.platform || 'instagram') === (platform || 'instagram')) {
      return { ...f, filters };
    }
...
ipcMain.handle('get-all-labels', () => store.get('allLabels', []));

ipcMain.handle('set-all-labels', (_e, labels) => {
  store.set('allLabels', labels);
  return labels;
});
...
ipcMain.handle('set-http-cache-enabled', (_e, enabled) => {
  store.set('httpCacheEnabled', enabled);
  return enabled;
});
...
ipcMain.handle('refresh-feed', async (_e, username, platform) => {
  ...
  await generateFeed(feedKey, profileData, store, platform, storedFeed);
