 ...
 */
async function generateFeed(username, profileData, store, platform, feedEntry) {
  ...
  const filters = feedEntry?.filters || {};
  let filteredPosts = profileData.posts;

  if (filters.includeKeywords?.length) {
    filteredPosts = filteredPosts.filter(p =>
      filters.includeKeywords.some(kw =>
  ...
  if (filters.excludeKeywords?.length) {
    filteredPosts = filteredPosts.filter(p =>
      !filters.excludeKeywords.some(kw =>
        (p.caption || '').toLowerCase().includes(kw.toLowerCase())
      )
    );
  }
  if (filters.mediaOnly) {
    filteredPosts = filteredPosts.filter(p => p.imageUrl || p.isVideo);
  }
  if (filters.minLikes > 0) {
    filteredPosts = filteredPosts.filter(p => (p.likes || 0) >= filters.minLikes);
  }

  const labels = feedEntry?.labels || [];
